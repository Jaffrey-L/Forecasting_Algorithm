...
def test_analysis_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ 分析状态查询成功: {status}")
            return status
        else:
            print(f"❌ 分析状态查询失败: {response.status_code}")
            return None
    except Exception as e:
        print(f"❌ 分析状态查询测试失败: {e}")
        return None
...
def test_analysis_result():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"✅ 分析结果查询成功: {result}")
            return result
        elif response.status_code == 400:
            print("⚠️  分析尚未完成")
            return None
        else:
            print(f"❌ 分析结果查询失败: {response.status_code}")
            return None
    except Exception as e:
        print(f"❌ 分析结果查询测试失败: {e}")
        return None
...
def test_wmape_calculation():
    ...
    print(f"测试WMAPE计算: 实际值={actual}, 预测值={predicted}, WMAPE={wmape:.4f} ({wmape * 100:.2f}%)")
    print("✅ WMAPE计算测试完成")
    return wmape
    ...
    if analysis_completed:
        ...
        if result:
            ...
            if 'average_wmape' in result:
                ...
                print(f"✅ 平均WMAPE: {avg_wmape:.4f} ({avg_wmape * 100:.2f}%)")
            else:
                print("⚠️  结果中没有平均WMAPE")
